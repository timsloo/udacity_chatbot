...
ChatLogic::ChatLogic()
{
    ...
    // create instance of chatbot: no longer handled by ChatLogic
...
ChatLogic::~ChatLogic()
{
    ...
    // delete chatbot instance: now owned by current node
    // delete _chatBot;  

    // delete all nodes: now handled by unique ptr
    ...
    // delete all edges: now handled by parent nodes
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                            auto edge = std::make_unique<GraphEdge>(id); // exclusive ownership (later transferred) 
                            ...
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // ownership is transferred 
                        }
